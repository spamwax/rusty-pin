...
impl CachedData {
    ...
    fn read_cached_pins(&mut self) -> Result<(), String> {
        ...
        let fp =
            File::open(&self.pins_cache_file).map_err(|e| e.description().to_owned())?;
    ...
    fn read_cached_tags(&mut self) -> Result<(), String> {
        ...
        let fp =
            File::open(&self.tags_cache_file).map_err(|e| e.description().to_owned())?;
        let mut de = Deserializer::from_read(fp);
        self.tags =
            Deserialize::deserialize(&mut de).map_err(|e| e.description().to_owned())?;
        Ok(())
    ...
    pub fn update_cache(&mut self, api: &api::Api) -> Result<(), String> {
        ...
        //
        let mut f =
            File::create(&self.pins_cache_file).map_err(|e| e.description().to_owned())?;
        ...
        //
        let mut f =
            File::create(&self.tags_cache_file).map_err(|e| e.description().to_owned())?;
